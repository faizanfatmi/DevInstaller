"""Modern post-installation completion screen.
...
shows per-status counts, and lists any failed items. A subtle fade-in keeps
...
class _StatCard(QWidget):
    """A compact card showing a single count with a coloured label."""

    def __init__(self, value: int, label: str, color: str, parent=None):
...
class CompletionDialog(QDialog):
    """Themed summary dialog shown when an installation batch finishes."""
    ...
    def __init__(
        ...
        subtitle = QLabel(
            "Some tools could not be installed. See details below."
            if has_failures
            else "Your development environment is ready to use."
        )
        ...
        cards.addWidget(_StatCard(skipped, "ALREADY PRESENT", FG_DIM))
